...
第二组和第三组已完成账户、合同和角色级结算规则；仍需完成附件。导入、导出作为后续固定接口/作业实现；信用额度留到 P8 金额模型。旧 `meta`、`advantages` 等无明确契约的 JSON 字段不原样迁移。旧合同的 `isCompanyTemplate`、多方主体评价等字段没有明确新业务契约，当前不迁移；旧账户软删除字段也不复制，停用由显式状态表达。
