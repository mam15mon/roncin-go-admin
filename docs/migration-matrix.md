...
| D07 | 费用与账单：`05-finance-billing.prisma`、`08-dictionaries.prisma` | BillingUnit、FeeSetting、TaxableService、Fee、Bill、BillFee、BillOrderLink、FinanceTag、VerificationRecord、PaymentRecord、InvoiceRecord、VerificationAllocation | 部分完成 | P8/P9 | 计费单位、费用项、应税服务目录、订单费用录入、应收/应付账单（含批量合并账单）、增值税发票登记与红冲、银行流水、核销与分摊、佣金核算以及订单/费用/账单标签已有契约、Ent 仓储、页面与验收脚本（`pnpm run acceptance:finance` 等）；对账单工作流、Fx 报价与审批仍按 D08/D09 推进，金额口径继续受 M-005 约束。 |
...
| D09 | 汇率：`06-fx.prisma` | ExchangeRateSetting、ExchangeRatePolicy、FxRateQuote、FxQuoteRevision、FxSnapshotRevision、BillFxSnapshotRevision、ChargeLineFxOverride、FxApproval | 部分完成 | P8 前置 | 汇率设置、自定义汇率设置、汇率时间标准、汇率导入批次与账单汇率快照已落地，费用录入支持手动汇率重写（override）；Fx 报价、报价修订与审批流仍未建模，费用/账单不得在页面临时计算的约束继续生效。 |
| D10 | 通知与工作流：`07-workflow-notification.prisma` | NotificationMessage、NotificationPreference、NotificationDeliveryLog、OrderReminderRule、OrderReminderDispatch、OrderTrackingEvent | 部分完成 | P10 | 钉钉通知意图、通知投递明细与后台任务 Worker 发送已落地（订单协作指派、钉钉用户授权两类模板，依赖后台任务基础设施）；消息记录、用户偏好、提醒规则、投递重试的完整口径与跟踪事件仍未开始，旧模型的无契约 JSON payload 不原样迁移。 |
| D11 | 主数据、参数与模板：`08-dictionaries.prisma` | CurrencyDictionary、CountryDictionary、Region、ContainerSpec、NumberRule、SerialSequence、PageTemplate、StatusTemplate、BusinessRuleTemplate、TemplateBundle、MilestoneTemplate、AirportDictionary、Unlocode* 等 | 部分完成 | P4 | 里程碑模板基础与首批组织级主数据目录、编号规则/并发序列、状态模板版本发布和默认版本切换已完成，但运行时里程碑首版属于 P7；结构化批量导入契约、OurAirports 机场同步和 UNECE UN/LOCODE 海港筛选同步已完成。BillingUnit、FeeSetting、TaxableService 调整到 D07/P8，页面模板按固定表单决策后置。 |
| D12 | 企业基础资源：`09-enterprise-resources.prisma` | BaseAddress、BaseConsignee、BaseShipper、BaseNotify、BaseResourcePartyRel、BaseNote、BaseTag、BaseBusinessCode、BaseImage、BaseTextSnippet | 已完成 | P4/P5 | 2026-08-29 已按《企业资源备忘录建设计划》交付：统一资源基表、分类详情、企业关联、地址类型与关联人员、标签组、三类单证主体两阶段导入和 S3 图片上传，旧 `partner_shipping_presets` 表已删除并切换到统一资源表，英文品名、HS 编码和唛头迁入内部详情表；2026-08-30 订单、费用与账单的批量标签关联基于同一标签模型落地。详见 `docs/plans/企业资源备忘录建设计划.md` 第 16 节与 `docs/plans/业务标签建设计划.md` 第 18 节。 |
| D13 | 幂等：`10-idempotency.prisma` | IdempotencyKey | 部分完成 | P1/P7 | 后台任务幂等已在 P7 落地：任务按“组织 + 幂等键”唯一，重复入队返回冲突；API 写入幂等（键作用域、请求摘要、过期、冲突响应）仍未开始。 |
| D14 | 数据维护作业：`11-data-maintenance-jobs.prisma` | RegionSyncJob、PortImportJob；另含 `UnlocodeImportBatch` | 部分完成 | P4/P7/P10 | P4 已提供最多 500 条、整批事务、显式 create-only/upsert 的结构化导入契约；机场与港口外部文件解析、冲突预览、幂等写入和来源哈希已通过独立同步命令接入。P7 已建立统一后台任务基础设施（Kind 区分主数据导入/UNLOCODE/订单提醒/集成、租约、重试上限、死信、条件回放与审计、追踪面板）；实际 worker 执行循环、导入批次查询与运维告警仍未接入，不得与普通 IntegrationTask 混成无类型任务。 |
...
第二组至第四组已完成账户、合同、角色级结算规则和附件元数据登记；真实对象存储上传、授权下载、删除、导入和导出作为后续固定接口/作业实现；信用额度留到 P8 金额模型。旧 `meta`、`advantages` 等无明确契约的 JSON 字段不原样迁移。旧合同的 `isCompanyTemplate`、多方主体评价等字段没有明确新业务契约，当前不迁移；旧账户和附件软删除字段也不复制，分别使用账户启停和后续文件生命周期策略表达。
