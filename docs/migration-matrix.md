...
| D05 | 订单扩展与执行：`04-order-extension.prisma` | OrderContainer、OrderCargoItem、OrderShippingDocument、OrderReleasePod、OrderCollaborator、OrderAbnormalCase、OrderAttachment、OrderAlertTask、OrderPersonnel 等 | 部分完成 | P7 | 附件元数据登记、协作人员闭环、集装箱、货物明细、提单与异常标记首版已完成：提单状态收紧为向前单步流转并带期望状态并发条件，(order_id, master_no) 唯一（修正旧系统可重复主单号），已放货提单禁改禁删；异常类型收敛为组织级 abnormal_case 主数据，已解决标记可重新激活；装载状态、提单-集装箱关联、计费重、AE 仓储字段按跟踪事件/P8/M-006 决策后置，不设空列（提单-集装箱关联已完成）。真实对象存储、放货凭证、跟踪事件、提醒仍未完成，需逐项建模。 |
...
| D14 | 数据维护作业：`11-data-maintenance-jobs.prisma` | RegionSyncJob、PortImportJob；另含 `UnlocodeImportBatch` | 部分完成 | P4/P7/P10 | P4 已提供最多 500 条、整批事务、显式 create-only/upsert 的结构化导入契约；P7 已建立统一后台任务基础设施（Kind 区分主数据导入/UNLOCODE/订单提醒/集成、租约、重试上限、死信、条件回放与审计、追踪面板），外部文件解析、实际 worker 执行循环与运维告警仍未接入，不得与普通 IntegrationTask 混成无类型任务。 |
