...
| D03 | 往来单位：`02-party-master.prisma` | Carrier、Party、PartyRole、CustomerProfile、SupplierProfile、AgentProfile、PartyContact、PartyRoleAccount、PartyContract、PartyRoleSettlementRule、PartyAttachment 及日志 | 部分完成 | P5/P6/P7 | 已完成组织级法人档案、客户/供应商/代理/承运人显式角色、多联系人、单一主联系人、别名、供应商黑名单、角色级结算账户、Partner 合同、角色级结算规则和附件元数据登记；默认账户有数据库条件唯一约束，停用账户不能设为默认，合同编号创建后不可变，结算规则组合键唯一且周期字段受领域校验，附件使用组织隔离和幂等键。信用额度留到 P8 金额模型；真实对象存储、授权下载、删除、导入和导出留到后续阶段。订单、费用和财务继续只引用 Partner 聚合 ID。 |
