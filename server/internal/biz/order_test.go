...
func (s *orderRepoStub) GetByIdempotencyKey(context.Context, uuid.UUID, string) (*Order, error) {
	if s.byIdempotencyKey != nil {
		return s.byIdempotencyKey, nil
	}
	return nil, nil
}
...
func TestOrderCreateBlocksExceededCustomerInInterventionMode(t *testing.T) {
	...
	reminderUsecase := NewOrderUsecase(&orderRepoStub{}, nil, &seaMasterBillRepoStub{}, nil, newReminderModeCreditControl(), nil)
...
func TestOrderRejectsInvalidAggregateAndDraftRollback(t *testing.T) {
	usecase := NewOrderUsecase(&orderRepoStub{current: &Order{BusinessType: OrderBusinessSE, FlowStatus: OrderFlowBooked, TerminationStatus: OrderTerminationActive, ClosureStatus: OrderClosureOpen, Version: 1}}, nil, &seaMasterBillRepoStub{}, nil, newReminderModeCreditControl(), nil)
	organizationID := uuid.New()
	actorID := uuid.New()
...
func TestOrderNormalizesBusinessFieldsAndRequiresCompleteCargoValue(t *testing.T) {
	...
	input := &Order{
		...
		UNNumber: "1234", HazardClass: "3", FactoryName: "  测试工厂  ", CargoReadyAt: "2026-08-23T12:00:00+08:00", ReceivedAt: "2026-08-23T10:00:00+08:00",
	}
	normalized, err := normalizeOrder(input, false)
	if err != nil {
		t.Fatalf("normalizeOrder() error = %v", err)
	}
	if normalized.CustomerReferenceNo != "CUST-001" || normalized.InternalReferenceNo != "INTERNAL-001" || normalized.ContractNo != "CONTRACT-001" || normalized.CargoValue != "100000.25" || normalized.CargoCurrency != "USD" || normalized.InsurancePremium != "100.50" || normalized.InsuranceCurrency != "CNY" || normalized.UNNumber != "1234" || normalized.HazardClass != "3" || normalized.FactoryName != "测试工厂" || normalized.ReceivedAt != "2026-08-23T10:00:00+08:00" {
...
func TestOrderClosureRequiresTerminalBusinessAndNoBlockers(t *testing.T) {
	...
	for _, testCase := range blockedCases {
		t.Run(testCase.name, func(t *testing.T) {
			usecase := NewOrderUsecase(&orderRepoStub{closureReadiness: testCase.readiness}, nil, &seaMasterBillRepoStub{}, nil, newReminderModeCreditControl(), nil)
...
func TestNormalizeOrderRequiresSEMasterBill(t *testing.T) {
	...
	input.ShippingLineID = &shippingLineID
