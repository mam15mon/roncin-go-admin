...
func (r *industryReferenceRepo) CreatePort(ctx context.Context, organizationID uuid.UUID, input *biz.Port, audit *biz.AuditEvent) (*biz.Port, error) {
	...
	if err := r.data.WithTx(ctx, func(tx *ent.Tx) error {
		...
		if createErr != nil {
			return mapEntError(createErr, nil, biz.ErrIndustryReferenceCodeExist)
		}
...
func (r *industryReferenceRepo) CreateAirline(ctx context.Context, _ uuid.UUID, input *biz.Airline, audit *biz.AuditEvent) (*biz.Airline, error) {
	var created *ent.Airline
	if err := r.data.WithTx(ctx, func(tx *ent.Tx) error {
		var createErr error
		created, createErr = tx.Airline.Create().
...
func (r *industryReferenceRepo) UpdateAirline(ctx context.Context, _ uuid.UUID, id uuid.UUID, input *biz.Airline, audit *biz.AuditEvent) (*biz.Airline, error) {
	var updated *ent.Airline
	if err := r.data.WithTx(ctx, func(tx *ent.Tx) error {
		existing, err := tx.Airline.Query().Where(airline.IDEQ(id)).Only(ctx)
		if err != nil {
			return mapEntError(err, biz.ErrIndustryReferenceNotFound, nil)
		}
...
func (r *industryReferenceRepo) ListShippingLines(ctx context.Context, organizationID uuid.UUID, options biz.IndustryReferenceListOptions) (*biz.ShippingLineList, error) {
	...
	// A 型全局主数据：全员同权可见，无组织过滤。
	query := client.ShippingLine.Query().WithContainerPrefixes(func(query *ent.ShippingLineContainerPrefixQuery) { query.Order(shippinglinecontainerprefix.ByPrefix()) })
	if options.Keyword != "" {
...
func (r *industryReferenceRepo) CreateShippingLine(ctx context.Context, _ uuid.UUID, input *biz.ShippingLine, audit *biz.AuditEvent) (*biz.ShippingLine, error) {
	var item *ent.ShippingLine
	if err := r.data.WithTx(ctx, func(tx *ent.Tx) error {
		created, err := tx.ShippingLine.Create().SetScacCode(input.SCACCode).SetNameZh(input.NameZH).SetNameEn(input.NameEN).SetCountryCode(input.CountryCode).SetNillableTrackingURL(input.TrackingURL).SetNillableAlliance(input.Alliance).SetSource(input.Source).SetSortOrder(input.SortOrder).SetEnabled(true).Save(ctx)
		if err != nil {
			return mapEntError(err, nil, biz.ErrIndustryReferenceCodeExist)
		}
		if err := replaceShippingLinePrefixes(ctx, tx, created.ID, input.ContainerPrefixes); err != nil {
			return err
		}
		item, err = tx.ShippingLine.Query().Where(shippingline.IDEQ(created.ID)).WithContainerPrefixes(func(query *ent.ShippingLineContainerPrefixQuery) { query.Order(shippinglinecontainerprefix.ByPrefix()) }).Only(ctx)
		if err != nil {
			return err
		}
		audit.Details["industry_reference.id"] = created.ID.String()
...
func (r *industryReferenceRepo) UpdateShippingLine(ctx context.Context, _ uuid.UUID, id uuid.UUID, input *biz.ShippingLine, audit *biz.AuditEvent) (*biz.ShippingLine, error) {
	var item *ent.ShippingLine
	if err := r.data.WithTx(ctx, func(tx *ent.Tx) error {
		existing, err := tx.ShippingLine.Query().Where(shippingline.IDEQ(id)).Only(ctx)
		if err != nil {
			return mapEntError(err, biz.ErrIndustryReferenceNotFound, nil)
		}
		...
		if err := replaceShippingLinePrefixes(ctx, tx, id, input.ContainerPrefixes); err != nil {
			return err
		}
		item, err = tx.ShippingLine.Query().Where(shippingline.IDEQ(id)).WithContainerPrefixes(func(query *ent.ShippingLineContainerPrefixQuery) { query.Order(shippinglinecontainerprefix.ByPrefix()) }).Only(ctx)
		if err != nil {
			return err
		}
...
func replaceShippingLinePrefixes(ctx context.Context, tx *ent.Tx, shippingLineID uuid.UUID, prefixes []string) error {
	for _, prefix := range prefixes {
		if _, err := tx.ShippingLineContainerPrefix.Create().SetShippingLineID(shippingLineID).SetPrefix(prefix).Save(ctx); err != nil {
			return mapEntError(err, nil, biz.ErrIndustryReferenceCodeExist)
		}
	}
	return nil
...
func shippingLineToBiz(item *ent.ShippingLine) *biz.ShippingLine {
	result := &biz.ShippingLine{ID: item.ID, SCACCode: item.ScacCode, NameZH: item.NameZh, NameEN: item.NameEn, CountryCode: item.CountryCode, TrackingURL: item.TrackingURL, Alliance: item.Alliance, Source: item.Source, SortOrder: item.SortOrder, Enabled: item.Enabled, CreatedAt: item.CreatedAt, UpdatedAt: item.UpdatedAt}
