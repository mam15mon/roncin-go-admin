...
func (_q *MembershipQuery) QueryRoleAssignments() *RoleAssignmentQuery {
...
func (_q *MembershipQuery) WithRoleAssignments(opts ...func(*RoleAssignmentQuery)) *MembershipQuery {
	query := (&RoleAssignmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MembershipQuery) loadRoleAssignments(ctx context.Context, query *RoleAssignmentQuery, nodes []*Membership, init func(*Membership), assign func(*Membership, *RoleAssignment)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(roleassignment.FieldMembershipID)
	}
