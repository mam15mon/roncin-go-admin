...
func (_q *PartnerQuery) QueryProfile() *PartnerProfileQuery {
	query := (&PartnerProfileClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *PartnerQuery) QueryInvoiceProfiles() *PartnerInvoiceProfileQuery {
...
func (_q *PartnerQuery) QueryAssignments() *PartnerAssignmentQuery {
...
func (_q *PartnerQuery) QueryFinanceBills() *FinanceBillQuery {
...
func (_q *PartnerQuery) QueryFinanceVerifications() *FinanceVerificationQuery {
...
func (_q *PartnerQuery) QueryIssuedSeaHouseBills() *SeaHouseBillQuery {
...
// QuerySeaOrderReassignments chains the current query on the "sea_order_reassignments" edge.
func (_q *PartnerQuery) QuerySeaOrderReassignments() *SeaOrderReassignmentEventQuery {
...
func (_q *PartnerQuery) WithProfile(opts ...func(*PartnerProfileQuery)) *PartnerQuery {
	query := (&PartnerProfileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PartnerQuery) WithInvoiceProfiles(opts ...func(*PartnerInvoiceProfileQuery)) *PartnerQuery {
	query := (&PartnerInvoiceProfileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PartnerQuery) WithAssignments(opts ...func(*PartnerAssignmentQuery)) *PartnerQuery {
	query := (&PartnerAssignmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PartnerQuery) WithFinanceBills(opts ...func(*FinanceBillQuery)) *PartnerQuery {
	query := (&FinanceBillClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PartnerQuery) WithFinanceVerifications(opts ...func(*FinanceVerificationQuery)) *PartnerQuery {
	query := (&FinanceVerificationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PartnerQuery) WithIssuedSeaHouseBills(opts ...func(*SeaHouseBillQuery)) *PartnerQuery {
	query := (&SeaHouseBillClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithSeaOrderReassignments tells the query-builder to eager-load the nodes that are connected to
// the "sea_order_reassignments" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PartnerQuery) WithSeaOrderReassignments(opts ...func(*SeaOrderReassignmentEventQuery)) *PartnerQuery {
	query := (&SeaOrderReassignmentEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PartnerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Partner, error) {
	...
	if query := _q.withInvoiceProfiles; query != nil {
		if err := _q.loadInvoiceProfiles(ctx, query, nodes,
			func(n *Partner) { n.Edges.InvoiceProfiles = []*PartnerInvoiceProfile{} },
			func(n *Partner, e *PartnerInvoiceProfile) {
				n.Edges.InvoiceProfiles = append(n.Edges.InvoiceProfiles, e)
			}); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withIssuedSeaHouseBills; query != nil {
		if err := _q.loadIssuedSeaHouseBills(ctx, query, nodes,
			func(n *Partner) { n.Edges.IssuedSeaHouseBills = []*SeaHouseBill{} },
			func(n *Partner, e *SeaHouseBill) {
				n.Edges.IssuedSeaHouseBills = append(n.Edges.IssuedSeaHouseBills, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := _q.withSeaOrderReassignments; query != nil {
		if err := _q.loadSeaOrderReassignments(ctx, query, nodes,
...
func (_q *PartnerQuery) loadProfile(ctx context.Context, query *PartnerProfileQuery, nodes []*Partner, init func(*Partner), assign func(*Partner, *PartnerProfile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Partner)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(partnerprofile.FieldPartnerID)
	}
...
func (_q *PartnerQuery) loadInvoiceProfiles(ctx context.Context, query *PartnerInvoiceProfileQuery, nodes []*Partner, init func(*Partner), assign func(*Partner, *PartnerInvoiceProfile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Partner)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.PartnerInvoiceProfile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(partner.InvoiceProfilesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *PartnerQuery) loadAssignments(ctx context.Context, query *PartnerAssignmentQuery, nodes []*Partner, init func(*Partner), assign func(*Partner, *PartnerAssignment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Partner)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(partnerassignment.FieldPartnerID)
	}
...
func (_q *PartnerQuery) loadFinanceBills(ctx context.Context, query *FinanceBillQuery, nodes []*Partner, init func(*Partner), assign func(*Partner, *FinanceBill)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Partner)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(financebill.FieldSettlementPartyID)
	}
...
func (_q *PartnerQuery) loadFinanceVerifications(ctx context.Context, query *FinanceVerificationQuery, nodes []*Partner, init func(*Partner), assign func(*Partner, *FinanceVerification)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Partner)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(financeverification.FieldSettlementPartyID)
	}
...
func (_q *PartnerQuery) loadIssuedSeaHouseBills(ctx context.Context, query *SeaHouseBillQuery, nodes []*Partner, init func(*Partner), assign func(*Partner, *SeaHouseBill)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Partner)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(seahousebill.FieldIssuerPartnerID)
	}
	query.Where(predicate.SeaHouseBill(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(partner.IssuedSeaHouseBillsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *PartnerQuery) loadSeaOrderReassignments(ctx context.Context, query *SeaOrderReassignmentEventQuery, nodes []*Partner, init func(*Partner), assign func(*Partner, *SeaOrderReassignmentEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Partner)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(seaorderreassignmentevent.FieldResponsiblePartnerID)
	}
	query.Where(predicate.SeaOrderReassignmentEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(partner.SeaOrderReassignmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.ResponsiblePartnerID
		if fk == nil {
			return fmt.Errorf(`foreign-key "responsible_partner_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
