...
func (FinanceVerification) Edges() []ent.Edge {
	return []ent.Edge{edge.From("organization", Organization.Type).Ref("finance_verifications").Field("organization_id").Unique().Required().Immutable(), edge.From("settlement_party", Partner.Type).Ref("finance_verifications").Field("settlement_party_id").Unique().Required().Immutable(), edge.From("reversed_by_user", User.Type).Ref("reversed_finance_verifications").Field("reversed_by").Unique(), edge.To("allocations", FinanceVerificationAllocation.Type)}
}
