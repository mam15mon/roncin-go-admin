...
func (OrderLifecycleEvent) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Enum("dimension").Values("FLOW", "TERMINATION", "CLOSURE", "ORIGIN").Immutable(),
		...
		field.String("reference_type").Optional().Nillable().MaxLen(64).Immutable(),
		field.UUID("reference_id", uuid.Nil).Optional().Nillable().Immutable(),
	}
}
