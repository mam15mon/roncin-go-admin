...
func (FinanceCashflow) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("amount").SchemaType(map[string]string{dialect.Postgres: "numeric(28,8)"}).Immutable(), field.String("exchange_rate").SchemaType(map[string]string{dialect.Postgres: "numeric(18,8)"}).Immutable(), field.Enum("exchange_rate_source").Values("SYSTEM", "MANUAL", "DERIVED", "WEEKLY", "INHERITED_LAST_WEEK", "BOC_SYNC").Immutable(), field.String("exchange_rate_date").NotEmpty().MinLen(10).MaxLen(10).Immutable(), field.UUID("exchange_rate_setting_id", uuid.Nil).Optional().Nillable().Immutable(), field.String("base_currency").NotEmpty().MinLen(3).MaxLen(3).Immutable(), field.String("base_amount").SchemaType(map[string]string{dialect.Postgres: "numeric(28,8)"}).Immutable(),
