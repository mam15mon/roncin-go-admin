...
// PartnerInvoiceProfile 保存往来单位的一套税务开票资料；历史发票仍使用自身快照。
...
func (PartnerInvoiceProfile) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.From("partner", Partner.Type).Ref("invoice_profiles").Field("partner_id").Unique().Required().Immutable(),
...
func (PartnerInvoiceProfile) Indexes() []ent.Index {
	return []ent.Index{
		index.Fields("organization_id", "partner_id"),
		index.Fields("organization_id", "partner_id", "invoice_title").Unique().StorageKey("partner_invoice_profile_title_key"),
		index.Fields("organization_id", "partner_id").Unique().StorageKey("partner_invoice_profile_default_key").Annotations(entsql.IndexWhere("is_default")),
