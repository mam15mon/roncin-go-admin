...
func (Partner) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("code").Optional().Nillable().MaxLen(64).Comment("客商单位编码；选填，仅用于操作人员搜索，创建后可修改"),
...
func (Partner) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.To("invoice_profiles", PartnerInvoiceProfile.Type),
		edge.To("assignments", PartnerAssignment.Type),
		edge.To("enterprise_resource_links", EnterpriseResourcePartner.Type),
