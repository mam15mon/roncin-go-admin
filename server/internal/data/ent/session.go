...
// was not loaded in eager-loading, or loaded but was not found.
func (e SessionEdges) OrganizationOrErr() (*Organization, error) {
...
func (*Session) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case session.FieldCreatedAt, session.FieldUpdatedAt, session.FieldExpiresAt, session.FieldLastSeenAt, session.FieldRevokedAt:
...
// QueryOrganization queries the "organization" edge of the Session entity.
func (_m *Session) QueryOrganization() *OrganizationQuery {
	return NewSessionClient(_m.config).QueryOrganization(_m)
}
