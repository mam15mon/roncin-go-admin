...
func (_q *SessionQuery) QueryOrganization() *OrganizationQuery {
...
func (_q *SessionQuery) WithOrganization(opts ...func(*OrganizationQuery)) *SessionQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := _q.withOrganization; query != nil {
		if err := _q.loadOrganization(ctx, query, nodes, nil,
			func(n *Session, e *Organization) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SessionQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Session, init func(*Session), assign func(*Session, *Organization)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Session)
	for i := range nodes {
...
func (_q *SessionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withOrganization != nil {
			_spec.Node.AddColumnOnce(session.FieldOrganizationID)
		}
	}
