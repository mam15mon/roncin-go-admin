...
func orderToAPI(item *biz.Order) *v1.Order {
	result := &v1.Order{
		...
		CustomerName: stringPtrIfNotEmpty(item.CustomerName), OriginLocationName: stringPtrIfNotEmpty(item.OriginLocationName), DestinationLocationName: stringPtrIfNotEmpty(item.DestinationLocationName),
		DischargeLocationName: stringPtrIfNotEmpty(item.DischargeLocationName), TransitLocationName: stringPtrIfNotEmpty(item.TransitLocationName),
		BookingAgentName: stringPtrIfNotEmpty(item.BookingAgentName), ForeignAgentName: stringPtrIfNotEmpty(item.ForeignAgentName), ShippingAgentName: stringPtrIfNotEmpty(item.ShippingAgentName),
		OperatorName: stringPtrIfNotEmpty(item.OperatorName), OperatorBranch: stringPtrIfNotEmpty(item.OperatorBranch), SalesName: stringPtrIfNotEmpty(item.SalesName), SalesBranch: stringPtrIfNotEmpty(item.SalesBranch), CreatorName: stringPtrIfNotEmpty(item.CreatorName),
...
func businessTagSummariesToAPI(items []*biz.BusinessTagSummary) []*v1.BusinessTagSummary {
	if len(items) == 0 {
		return nil
	}
