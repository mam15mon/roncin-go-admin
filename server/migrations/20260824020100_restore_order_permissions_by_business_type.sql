-- 修复上一迁移的正则表达式转义，恢复四类订单权限。
CREATE TEMP TABLE order_business_types ("code" varchar NOT NULL, "name" varchar NOT NULL) ON COMMIT DROP;
INSERT INTO order_business_types ("code", "name") VALUES
  ('se', '海运出口（SE）'), ('si', '海运进口（SI）'), ('ae', '空运出口（AE）'), ('ai', '空运进口（AI）');

CREATE TEMP TABLE order_permission_definitions ("operation" varchar NOT NULL, "name" varchar NOT NULL, "resource" varchar NOT NULL, "description" varchar NOT NULL) ON COMMIT DROP;
INSERT INTO order_permission_definitions ("operation", "name", "resource", "description") VALUES
  ...
  ('milestone.read', '查看里程碑', '里程碑', '查看订单里程碑'), ('milestone.set', '设置里程碑', '里程碑', '完成、跳过或重置订单里程碑'), ('attachment.read', '查看附件', '附件', '查看订单附件'), ('attachment.register', '登记附件', '附件', '登记订单附件元数据'),
  ...
  ('release_pod.read', '查看放货凭证', '放货', '查看订单放货凭证'), ('release_pod.create', '新增放货凭证', '放货', '新增订单放货凭证'), ('release_pod.update', '编辑放货凭证', '放货', '修改订单放货凭证'), ('release_pod.transition', '流转放货状态', '放货', '执行订单放货状态流转'), ('release_pod.delete', '删除放货凭证', '放货', '删除订单放货凭证');

INSERT INTO "permissions" ("id", "created_at", "updated_at", "key", "name", "group", "description")
SELECT md5(format('business.order.%s.%s', business_type."code", definition."operation"))::uuid, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP,
       format('business.order.%s.%s', business_type."code", definition."operation"), definition."name",
       format('订单管理 · %s · %s', business_type."name", definition."resource"), definition."description"
FROM order_business_types AS business_type CROSS JOIN order_permission_definitions AS definition
...
JOIN "permissions" AS permission ON permission."key" LIKE 'business.order.%'
