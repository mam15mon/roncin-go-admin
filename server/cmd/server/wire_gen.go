...
func wireApp(confServer *conf.Server, confData *conf.Data, security *conf.Security, logger *slog.Logger) (*kratos.App, func(), error) {
	...
	financeBillUsecase := biz.NewFinanceBillUsecase(financeBillRepo, orderConfigUsecase)
	financeInvoiceRepo := data.NewFinanceInvoiceRepo(dataData)
	financeInvoiceUsecase := biz.NewFinanceInvoiceUsecase(financeInvoiceRepo, orderConfigUsecase)
	financeCashflowRepo := data.NewFinanceCashflowRepo(dataData)
	financeCashflowUsecase := biz.NewFinanceCashflowUsecase(financeCashflowRepo, orderConfigUsecase)
	verificationRepo := data.NewVerificationRepo(dataData)
	verificationUsecase := biz.NewVerificationUsecase(verificationRepo, orderConfigUsecase)
	...
	settlementService := service.NewSettlementService(settlementUsecase, financeBillUsecase, financeInvoiceUsecase, financeCashflowUsecase, verificationUsecase, commissionUsecase)
