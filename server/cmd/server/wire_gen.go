...
func wireApp(confServer *conf.Server, confData *conf.Data, security *conf.Security, logger *slog.Logger) (*kratos.App, func(), error) {
	...
	commissionUsecase := biz.NewCommissionUsecase(commissionRepo, orderConfigUsecase, dataData)
	orderService := service.NewOrderService(orderUsecase, commissionUsecase)
	...
	seaDocumentChangeRepo := data.NewSeaDocumentChangeRepo(dataData)
	seaDocumentChangeUsecase := biz.NewSeaDocumentChangeUsecase(seaDocumentChangeRepo)
	seaDocumentService := service.NewSeaDocumentService(seaDocumentUsecase, seaDocumentChangeUsecase)
	seaSharedContainerRepo := data.NewSeaSharedContainerRepo(dataData)
	seaSharedContainerUsecase := biz.NewSeaSharedContainerUsecase(seaSharedContainerRepo)
	seaSharedContainerService := service.NewSeaSharedContainerService(seaSharedContainerUsecase)
	...
	exchangeRateQuoteProvider := data.NewExchangeRateQuoteProvider()
	exchangeRateUsecase := biz.NewExchangeRateUsecase(exchangeRateRepo, exchangeRateQuoteProvider)
	...
	autoOrderLockRepo := data.NewAutoOrderLockRepo(dataData)
	autoOrderLockUsecase := biz.NewAutoOrderLockUsecase(autoOrderLockRepo)
	orderFeeUsecase := biz.NewOrderFeeUsecase(orderFeeRepo, exchangeRateUsecase, financeCustomSettingUsecase, partnerCreditUsecase, autoOrderLockUsecase, logger)
	businessTagUsecase := biz.NewBusinessTagUsecase(businessTagRepo)
	orderFeeSupplementRequestRepo := data.NewOrderFeeSupplementRepo(dataData)
	orderFeeSupplementUsecase := biz.NewOrderFeeSupplementUsecase(orderFeeSupplementRequestRepo, orderFeeUsecase, dataData)
	...
	verificationUsecase := biz.NewVerificationUsecase(verificationRepo, exchangeRateUsecase, dataData, autoOrderLockUsecase, logger)
	financeNettingRepo := data.NewFinanceNettingRepo(dataData)
	financeNettingUsecase := biz.NewFinanceNettingUsecase(financeNettingRepo, dataData, autoOrderLockUsecase, logger)
	financeCommissionApplicationRepo := data.NewFinanceCommissionApplicationRepo(dataData)
	financeCommissionApplicationUsecase := biz.NewFinanceCommissionApplicationUsecase(financeCommissionApplicationRepo)
	...
	settlementService := service.NewSettlementService(settlementUsecase, financeBillUsecase, financeInvoiceUsecase, financeCashflowUsecase, verificationUsecase, financeNettingUsecase, commissionUsecase, financeCommissionApplicationUsecase, feeLedgerPreferenceUsecase, financeCustomSettingUsecase, businessTagUsecase, partnerAccountUsecase)
	workbenchRepo := data.NewWorkbenchRepo(dataData)
	workbenchUsecase := biz.NewWorkbenchUsecase(workbenchRepo)
	workbenchService := service.NewWorkbenchService(workbenchUsecase, financeCommissionApplicationUsecase)
	...
	exchangeRateReminderRepo := data.NewExchangeRateReminderRepo(dataData)
	exchangeRateReminderUsecase := biz.NewExchangeRateReminderUsecase(exchangeRateReminderRepo)
	exchangeRateReminderWorker := server.NewExchangeRateReminderWorker(exchangeRateReminderUsecase, logger)
	app := newApp(logger, grpcServer, httpServer, notificationWorker, dingTalkApprovalWorker, objectDeletionWorker, exchangeRateReminderWorker)
	return app, func() {
		cleanup()
	}, nil
}
