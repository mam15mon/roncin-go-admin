...
- [x] 第二十二批：`partner_secondary.go` 账户与合同标准 NotFound 分支
      改用公共 `mapEntError`
- [x] 第二十三批：`exchange_rate_import.go` 批次 NotFound 与导入幂等约束
      改用公共 `mapEntError`
- [x] 第二十四批：`admin_organization.go` 组织 NotFound 与编码约束
      改用公共 `mapEntError`
...
- [x] 第三十一批：`finance_bill.go` 单据、单据行与批次保存约束
...
- [x] 第三十四批：`admin_user_membership.go` 用户与成员关系标准 NotFound
      改用公共 `mapEntError`，保留已有关系启用/不存在创建的分流语义
- [x] 第三十五批：`admin_user.go` 用户、成员关系与用户名约束的标准错误
...
- [x] 第三十八批：`order_fee.go` 读取、目录快照与创建阶段的标准
...
### C8. react-query 决策（低）
- [x] 源码仍为 ProTable 请求与模块级候选项缓存，未出现需要复杂服务端状态管理的
      场景；删除未使用的 react-query 直依赖及 Umi 插件配置，避免同时安装 v4
      传递依赖与 v5 直依赖。后续出现跨页面缓存、失效与竞态需求时再统一引入
...
- [x] 已复核并实施跨仓储事务首个试点：订单创建原本已经满足事务原子性；新增
      Biz `Transactor`、Data 共享事务上下文和 Wire 注入，单账单创建的汇率快照与
      账单写入现已共享事务。外部副作用边界及后续阶段见
      [跨仓储事务与外部副作用边界设计](docs/cross-repository-transactions.md)。
