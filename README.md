...
- **安全与权限体系**：基于 RBAC 的 200+ 个细粒度业务权限清单、支持企业微信与钉钉扫码登录。
- **多组织工作区**：顶栏切换当前组织，页签与表单草稿按用户与组织双重身份隔离；角色可按权限追加只读或可写的跨组织访问范围（订单、往来单位与账单已接入），读取与写入范围分别校验。
...
# 2. 初始化系统管理工作台、管理员及独立公司（不自动授予公司成员资格）
pnpm run bootstrap:admin
```
> **初始管理员账号**：`admin`  
> **初始管理员密码**：由 `.env.local` 中的 `BOOTSTRAP_ADMIN_PASSWORD` 指定（必须至少 12 位，命令不会使用任何默认密码）。

公共港口、机场、船公司等行业资料通过 `pnpm run sync:all` 导入。系统管理员在
“系统管理”工作台维护公共资料，并显式分配公司成员及角色；进入公司后使用该公司权限。

开发演示数据使用 `pnpm run sync:dev`，仅用于开发数据库。它写入独立公司的测试员工、
客户、订单和财务数据；不应在生产库执行。系统费用模板用于新公司创建时初始化，
后续修改模板不自动覆盖已有公司的费用配置。
