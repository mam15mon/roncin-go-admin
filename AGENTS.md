...
- **时间显示规范 (Time Display Standards)**：
  - 界面展示与导出文件名中的时间一律输出人可读的本地时区格式，统一复用
    `web/src/utils/format.ts` 的 `formatDate`（`date` / `minute` / `datetime`
    三档精度）或 ProTable 列的 `valueType: 'dateTime'`；禁止把后端返回的
    ISO 8601 / UTC 原始字符串（如 `2026-09-21T14:44:52Z`）直接渲染到界面。
  - 禁止用 `slice(0, 16).replace('T', ' ')` 之类的字符串截取冒充时间格式化：
    该写法不换算时区，会把 UTC 时刻当本地时刻展示（差 8 小时）；纯日期字段
    （`YYYY-MM-DD`，无时间部分）不受此限。
  - 新增时间字段展示时按交互密度选择精度：列表列与摘要卡片用 `minute`，
    详情抽屉与审计类信息用 `datetime`，同一实体在各入口的展示保持口径一致。
- **金额显示规范 (Amount Display Standards)**：
  - 金额类字段（费用金额、折本币、已核销/未核销、汇兑损益、提成、税额、
    信用额度、余额等）在界面展示一律统一复用 `web/src/utils/format.ts` 的
    `formatAmount`（千分位 + 固定两位小数，`15362.5` → `15,362.50`）；
    后端 decimal 字符串带 8 位尾零（如 `15362.50000000`），禁止直接渲染到界面。
  - 禁止用 `toFixed(8)`、字符串截取或各页自写正则去尾零冒充金额格式化；
    提成模块的 `decimalText` 仅是 `formatAmount` 的展示别名，不得再用于
    汇率、比例等非金额字段。
  - 比率与数量类字段（汇率、提成比例 %、数量、单价）统一用 `trimDecimal`
    （去尾零、变长小数，如 `6.7183`、`3.5`）；强制两位小数会使汇率失真。
  - CSV/Excel 导出的金额单元格用两位小数但**不带千分位**
    （`Number(value).toFixed(2)`），避免逗号被表格软件按文本解析；
    表单输入框与提交载荷保持原始字符串，展示格式化只作用于渲染层。
  - 空值口径：`formatAmount(null/undefined)` 展示 `-`；统计汇总等需要
    显式零的场景用 `formatAmount(value ?? '0')` 展示 `0.00`，同一实体在
    列表、详情、导出各入口的金额口径保持一致。
