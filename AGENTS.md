...
- agy 实施过程中的第一轮测试只运行与当前改动直接相关的最小检查，禁止在每轮
  修改后重复执行完整前端测试、`check:web` 或构建。普通本地提交本身不构成运行
  完整门禁的理由；只有在整个任务或已批准阶段完成、准备最终验收/归档/推送，
  或变更属于高风险跨层、契约、权限、构建配置时，才按风险执行一次完整验证。
  若由 Codex 主会话统筹，Codex 检查真实差异和跨层数据流后再行提交。
