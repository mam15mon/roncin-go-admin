...
- **订单地点引用**：海运摘要按已保存港口 ID 解析时，允许 NULL 共享基线或当前公司记录；不能只查当前公司，也不能用同码遮蔽让既有引用消失。选择器与订单地点写入校验遵循本地同码优先，并要求启用；再次保存时若原共享 ID 已被本地同码行遮蔽，须重选当前候选，不自动替换 ID；其他公司的私有港口/机场始终不可用。共享起运、卸货、中转港均应覆盖回归，避免港口解析失败被映射为“海运主单不存在”而阻断整页订单。
