...
| 角色 A 有权限且 `data_scope=all` | readable / writable 均为全部启用组织 |
| 停用组织 | 不进入 readable / writable |
...
- Good：天津角色持有 `data_scope=organization_tree` 的订单读取权限时，读取范围为
  天津及其启用中的下级组织。
- Base：`data_scope=organization` 时按当前组织访问；`all` 时按全部启用组织访问。
- Bad：订单角色只有查看权限，财务角色范围更大；把两者拍平后允许查看范围外订单。
- Bad：先按 ID 全局读取实体，再在 Service 内存中判断组织；这会扩大数据暴露面，且
  子资源、预加载或审计很容易在判断前越界。

### 6. 必需测试

- Resolver 单元测试：四种 `data_scope`、停用组织、无权限拒绝、同权限多角色并集，
  以及不同权限角色不得借用范围。
- 角色管理测试：创建、更新和成员角色分配均覆盖跨角色借权反例。
...
会原样输出小时数、日期位产出 `00`，生成 `YYYY-MM-00` 非法日期；下游月窗口
解析随之失败，症状是“候选为 0 / 月度窗口为空”而非报错（源自月度提成申请
任务性能探针的实际教训）。

```go
// 错误：15 是小时占位符，产出非法日期 "2026-09-00"
month := time.Now().Format("2006-01-15")
// 正确
month := time.Now().Format("2006-01-02")
// 推导月初/月末用 AddDate，避免手拼布局
