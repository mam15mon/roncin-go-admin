...
users.dingtalk_requested_organization_id  -- 通道 B 自选/Token锁定目标组织（空=总部兜底）
```
...
- **向上追溯通知兜底（Escalation）**：审批通知路由目标组织内持权审批人；若目标组织
  审批人集合为空，沿 `parent_id` 逐级向上追溯首个存在有效审批人的祖先节点（直至总部根节点），
...
- **通道 B 认领**：注册确认可选目标组织（>1 时下发候选，有 Token 时由 Token 锁定隐藏下拉），未选走总部兜底；审批通知
...
| 通道 B 未选组织且无 Token | 总部兜底，通知总部管理员 |
