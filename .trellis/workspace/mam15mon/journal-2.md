...
## 2026-09-24 全站表头设置界面统一（09-24-unified-table-headers）

**Completed**

- 交付：`components/ui/column-settings/` 公共能力（ColumnSettingsModal 标准版+advanced 增强版、
  ColumnSettingsEntry 统一齿轮入口、useColumnSettings 列数组适配钩子、用户+组织+表格标识
  隔离的本地偏好存取）；5 个共享模板内置接入；订单费用与财务费用台账两套旧弹窗迁移；
  admin/客商/企业资源/财务/订单/工作台/建账工作台约 60 个页面级表格全部接入，
  ProTable 原生列设置入口全部关闭，普通 Table 与导入预览按表格上方右侧入口补齐。
- 教训 1：antd 按钮两个汉字文本会插入空格，可访问名称是「保 存」，RTL 断言要用 `/^保\s*存$/`。
- 教训 2：含 JSX 的模块必须 `.tsx`；`.ts` 下 esbuild 直接语法报错。
- 教训 3：与并行工作线（e50b62f2 金额显示统一）同仓并行时，共享文件会出现混合 hunk——
  整文件入库 + 提交说明注明搭车内容，比逐 hunk 拆分更稳；对应测试断言要随快照同步，
  否则提交快照自身就是红的。
- 教训 4：`useColumnSettings` 这类读取列数组的 hook，内部必须用派生签名（key+约束拼接）
  做副作用依赖，不能依赖调用方数组引用，否则内联列定义页面会渲染死循环。
- 验证：`check:web` 全绿（183 文件/1072 用例）；浏览器人工验收因本会话无浏览器后端未执行。
