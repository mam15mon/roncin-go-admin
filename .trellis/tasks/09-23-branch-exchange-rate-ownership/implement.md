...
- [ ] 确认取消自动历史周继承、缺失提示「请先维护汇率」的最新决定；已有手工覆盖权限保持，公共基线仅留作旧事实。
...
- [ ] 保留同币种 1、业务日期所在周公司配置、双轨方向取价与原有合法手工覆盖；新计算不产生 INHERITED_LAST_WEEK，旧来源仅用于快照展示。
...
发现需要删除公共记录、批量复制公共价格、恢复历史周继承、增加复制上周、改变人工覆盖或新增锁协议时，先回到设计审阅，不能自行扩大需求。
