...
1. **默认账期回填位置**（推荐方案 A）：
   - **方案 A（推荐）**：服务端在创建账单请求未显式提供账期时兜底注入默认值（散客恒 0；正式客户取规则值）。默认值注入不属于拦截，覆盖 API 直录路径；
   - 方案 B：纯前端预填，服务端不动。API 直录不享受默认值。
2. **账单编辑场景**：核实账单创建后的可编辑状态范围（账单状态机 + `finance_locked`），在允许编辑账期 / 到期日的状态中同样触发散客预警与超额预警展示；不可编辑状态无需处理。
