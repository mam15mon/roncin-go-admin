...
- [ ] P0 四个子任务全部完成归档，四个大文件均 <1500 行，相关定向测试全绿。
- [ ] P1 七个子任务全部完成归档，涉及文件均降到职责单一的可读规模。
- [ ] 全程零行为变更：无契约/生成物/迁移变化，无公开 API 变化，
      `pnpm run check` 最终门禁一次通过。
- [ ] P2 观察名单复查一次：对仍 >1000 行的文件逐个给出拆或不拆的结论记录。
- [ ] AGENTS.md / `.trellis/spec/` 中引用的文件路径（如 `order_write.go`
      并发范本）全部与拆分后实况一致。
