...
- [ ] 确认取消自动历史周继承、缺失提示「请先维护汇率」的最新决定；手工输入统一要求修改汇率权限，公共基线仅留作旧事实。
...
- [ ] Manifest 将汇率权限纳入公司经营范围，更新描述、工作台权限投影和授权测试；保留维护权限键、退役独立 override 权限并同步生成物与角色种子，避免波及费用目录。
...
- [ ] 删除公司历史周继承及公共基线直连/历史/套算路径及内部 PivotCurrency 参数，同步接口实现和测试桩。
- [ ] 保留同币种 1、业务日期所在周公司配置、双轨方向取价与基于修改汇率权限的显式手工输入；新计算不产生 INHERITED_LAST_WEEK，旧来源仅用于快照展示。
...
- [ ] 服务端返回汇率缺失错误，前端提示「请先维护汇率」并阻止依赖自动折算的提交；移除沿用上周提示，不增加复制上周功能。
- [ ] 修改历史汇率导入工具及开发种子，禁止无公司目标写公共基线；不自动运行导入或清理数据。

- [ ] 所有手工输入入口统一校验 FinanceExchangeRateUpdate：包括费用、资金流水和批量路径；无权限伪造请求拒绝，有权限仍需单据权限与公司范围。区分保留旧手工快照和实际修改汇率。
...
| 目标业务周缺失、有上周/更早/未来公司行 | 缺失错误，前端提示「请先维护汇率」，不自动提交 | 4 |
| 补录历史业务、有对应周公司行 | 使用业务日期所在周，不取当前周 | 3、4 |
| 历史开口行跨越自然周 | 不得借覆盖条件变相继承 | 4 |
...
| 无修改权限/仅旧覆盖权限输入手工汇率 | 前端不可编辑，服务端拒绝伪造；缺配置提示「请先维护汇率」 | 7 |
| 有修改权限且本公司合法操作 | 可显式输入，含缺周配置场景；记录 MANUAL，不自动补写周配置 | 7 |
| 无修改权限只编辑无关字段 | 不篡改也不无故重算已有手工快照 | 6、7 |
...
发现需要删除公共记录、批量复制公共价格、恢复历史周继承、增加复制上周、扩大手工输入权限或新增锁协议时，先回到设计审阅，不能自行扩大需求。
