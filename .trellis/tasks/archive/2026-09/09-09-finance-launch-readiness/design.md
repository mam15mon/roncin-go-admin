...
每张 `FinanceBill` 仍只有一个 `currency`，并固定等于该叶子全部费用的计价币种。`base_currency`
是组织本位币，`exchange_rate` 是账单日期对应的“账单币种 → 组织本位币”汇率，
`base_currency_amount` 是账单金额按该汇率计算的快照。

`FinanceBillLine` 继续使用现有币种、含税/未税/税额和本位币字段保存建账时的费用事实；不同
费用币种在进入账单前已经被强制拆开，不新增“原币 / 目标账单币种”双套字段，也不存在行级
跨币种转换率或人工转换率。服务端以同一账单日汇率计算所有明细本位币金额，最后一条明细吸收
确定性舍入尾差，使明细本位币合计严格等于账单头本位币金额。
...
结算单位和费用币种都不作为可关闭布尔值；二者始终参与分组。普通模式的全部费用必须方向相同；
对冲模式必须同时包含应收和应付，并在后续分别形成原始账单。契约不接收 `split_by_currency`、
目标账单币种或费用原币到目标币种的汇率覆盖，从类型层面消除跨币种合单状态。

预览请求按叶子携带账单日期、结算账户和预计开票配置。预览响应返回扁平叶子分组；每个叶子包含
完整分组事实、费用明细、账单币种合计、账单日到本位币的汇率快照、候选默认账户和 `group_key`。
...
+ settlement_party_id
+ base_currency
+ bill_currency（恒等于费用币种）
+ tax_rate（启用时）
+ order_id（启用时）
```

费用币种永久参与分组；不存在跨费用币种折算后合成一个叶子的路径。

预览令牌绑定：费用 ID/版本/状态/金额、分组策略、固定账单币种、账单日期、账单日汇率、账户、
预计开票配置和所有会影响金额或分组的配置。创建事务内重读费用和账户并重新计算；摘要不同则
返回冲突。
...
普通工作台固定按结算单位和费用币种拆分；可选策略默认为分订单开启、分税率关闭。服务端叶子在
前端投影为：

```text
结算单位 → 币种（固定）→ 税率（启用时）→ 订单（启用时）
```
...
层级只用于导航，不成为第二套分组算法。页面不提供“分币种”开关或账单币种编辑器；任何影响
分组事实的可选策略修改仍须由服务端重建预览并轮转预览令牌。
