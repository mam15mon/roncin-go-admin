...
在除已批准的七字段存量缺陷修复外保持 SE 行为等价的前提下，为订单列表、新建、详情、费用
页及其数据 Hook 建立唯一的订单类型注册入口，使路由类型、业务枚举、运输方式、表单适配和
详情专属能力不再散落在页面条件分支中。
...
- 除已批准的七字段存量缺陷修复外，本任务不得改字段名、校验规则、日期转换、空值处理、人员
  装配、海运主分单或箱量请求语义。
...
- [x] **AC-1**：唯一注册表只注册 `sea-export`，直接 kind 与合法订单路径均解析到同一对象；
  空值、未知类型、SI/AE/AI/LAND/RAIL 均返回 `undefined`。
- [x] **AC-2**：列表、新建、详情和费用页遇到未注册类型均呈现明确 404，不发起目标订单或
  类型主数据请求，不选择任一默认模板。
- [x] **AC-3**：产品代码中不存在旧 `ORDER_KIND_CONFIGS`、`OrderKindConfig`、
  `parseOrderKind`、兼容桥、重复 `OrderKind` 联合、本地订单类型 `kindMap` 或相关 `as any`。
- [x] **AC-4**：新建页只通过 SE 表单适配器取得 Sections、默认值和创建 Payload；除已批准的
  七字段存量缺陷修复外，创建请求与重构前在字段、默认值、人员、海运单证和箱量上行为等价。
- [x] **AC-5**：详情页只通过 SE 表单适配器取得 Sections、初始值和更新 Payload；除已批准的
  七字段存量缺陷修复外，更新请求与重构前行为等价，且不再通过字段存在性推断运输方式。
- [x] **AC-6**：通用详情页不直接依赖列出的 Sea 覆盖层或 sea change service；SE 扩展独立
  持有其请求与本地状态，并保持拆票、改配、共享航次、共享箱、同批订单和历史入口行为。
- [x] **AC-7**：拆票/改配按钮的显示、顺序、颜色、禁用原因、权限、锁单保护与路由不变；
  类型扩展的迟到请求不能污染切换后的订单身份。
- [x] **AC-8**：订单列表标题、业务类型显示、筛选、主数据加载、费用页权限与详情导航继续
  使用 SE 元数据，且公共列表模板不再维护业务类型名称表。
- [x] **AC-9**：前端未新增与权限 Manifest 重复的 capability 表；SE 专属操作仍以
  `access.canOrder`、`allowedActions` 和锁单策略共同控制。
- [x] **AC-10**：`OrderFormTemplate` 生命周期契约和详情统一刷新令牌实现无改动；现有同订单
  乱序、A/B 和 ABA 测试保持通过。
- [x] **AC-11**：相关定向 Vitest、修改文件 Biome、`pnpm --dir web tsc`、
  `git diff --check` 通过；任务最终执行一次 `pnpm run check:web` 并通过。
- [x] **AC-12**：实现、测试与项目规范描述同一所有权边界，不存在未记录的兼容分支或提前
  注册未实现订单类型。
...
本轮独立复核只批准了一项对既有 SE 行为的偏离：修复存量转换缺陷，补齐详情回填与更新请求
遗漏的七个字段：`unNumber`、`hazardClass`、`factoryName`、`cargoReadyAt`、
`declarationCutoffAt`、`receivedAt`、`specialRequirements`。修复同时补充完整夹具，明确验证
日期字段的 Dayjs/ISO 转换和空值 `undefined` 语义。

除上述七字段缺陷修复外，SE 的创建、详情、更新、列表、费用、按钮、权限、锁单、草稿、
刷新令牌和错误行为均要求与重构前等价。land/rail 本轮只做未开放资源的 fail-closed 边界与
竞态测试，不代表提前开放或改变业务行为。

本轮先运行任务实施计划中列出的定向 Vitest、修改文件 Biome、`pnpm --dir web tsc` 和
`git diff --check`。主会话随后在最终验收阶段执行 `pnpm run check:web` 并通过：permission
keys 295、Proto constants 64 enums/7 reason domains、Biome 462 files（`.agents/skills/ant-design`
的权限诊断未导致失败）、tsc 通过，Vitest 98 files/488 tests passed，耗时 69.66s。
