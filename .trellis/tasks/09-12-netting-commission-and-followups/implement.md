...
7. **PreviewSplit**：GetSplitContext 后对来源订单执行内容门禁，未通过追加
   ValidationErrors（design 2.5），不改 Execute 语义。
8. **admin 测试迁移**：TestAdminEmployeeLifecyclePostgres 夹具迁 getIntegrationData。
9. **测试**：
...
10. **验证**：build/vet/test 全量 + data 集成全量（隔离 schema）+ migrate:dev。
...
2. spec：finance-commission-lock.md 补来源模型与对冲冲减；order-lock spec 的预览拦截
   家族补 PreviewSplit；index 登记；
3. 提交分组：
   - `feat(commission): 对冲结清触发提成（来源二选一+计算管线推广+冲减联动）`
   - `fix(order-change): 拆票预览提前拦截订单门禁`
