...
### 阶段 A：员工级规则 Schema 与迁移

- [ ] 修改 `server/internal/data/ent/schema/finance_commission_rule.go`：增加员工外键/边、CHECK 和索引；组织/用户反向边保持生成关系一致。
- [ ] 增加正式 SQL 迁移：新增可空员工列与 `NO ACTION` 外键，停用旧空员工规则，删除组织内规则名唯一索引，增加员工规则查询索引及同名 CHECK。
...
- [ ] 提交：`feat(finance): 将提成规则关联到具体员工`。

### 阶段 B：提成规则领域逻辑与并发控制

- [ ] 扩展 `internal/biz/finance_commission.go` 的规则领域对象、过滤器和输入；新增员工成员资格、区间重叠及旧规则只读错误。
- [ ] 修改 `internal/data/finance_commission.go`：规则列表关联员工，关键字覆盖员工；写入统一按 Membership → Rule 固定锁序执行，并在锁内校验重叠。
- [ ] 创建/更新/重新启用均校验当前组织有效成员；驱动约束与版本冲突映射到稳定领域错误。
- [ ] 实现员工级规则 CRUD 定向测试；实现 PostgreSQL 并发集成测试验证重叠规则只有一个成功。
- [ ] 提交：`feat(finance): 强制员工提成规则区间唯一`。

### 阶段 C：计提契约与规则自动解析

- [ ] 修改 `server/api/finance/v1/settlement.proto`：规则输入/输出增加员工；候选与预览/创建改为员工 + 人员身份，由服务端解析规则。
- [ ] 重新生成服务端 API、OpenAPI 与前端客户端。
- [ ] 重构核销及对冲候选：按来源订单归属发现员工身份组合，再按来源日期解析唯一员工规则。
- [ ] 预览、创建和确认重算均校验规则员工、身份、日期与归属；保留现有来源指纹、幂等和活跃唯一索引。
- [ ] 更新规则管理抽屉和提成创建弹窗：员工关键字选择、候选合并展示、旧规则只读。
- [ ] 补充 Biz/Data/Service 与前端定向测试，覆盖多身份、固定薪无候选、日期边界、核销/对冲同构和历史快照。
- [ ] 提交：`refactor(finance): 按员工身份自动解析提成规则`。
...
- [ ] 实现 `has_commission_eligibility`：员工级当前/未来规则或非取消历史提成/调整；订单归属不单独开启。
- [ ] 实现本人提成分桶、预计机会、应收未结、近期海运出口订单和可靠作业待办；不同币种不裸合计。
- [ ] 复用补录申请实时审批资格，聚合财务提成/冲减摘要；只返回计数、少量记录和现有页面入口。
- [ ] 补 Biz/Data/Service 测试及权限矩阵，验证固定薪、未来规则、历史规则、仅取消记录、财务权限和组织切换。
...
- [ ] 门禁为真时实现提成状态卡、预计说明、历史调整、回款和下钻；未来规则展示生效日期。
...
- [ ] 在 PostgreSQL 集成环境验证正式迁移、规则并发、工作台聚合和订单摘要查询计划；记录无法运行的集成项。
...
server/migrations/<timestamp>_employee_commission_rules.sql      # 新增
