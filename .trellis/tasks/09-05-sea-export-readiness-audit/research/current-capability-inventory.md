...
除此之外，节点、异常、放单回单与旧 HBL 模型残留存在明显 P1 缺口；并发现两个可复现的前端参数缺陷，以及一个已由真实 PostgreSQL 完整集成套件稳定触发的拆票并发错误语义缺陷。建议按文末优先级与验收条件切分正式启用范围。
...
| 订单附件无可用上传/查看/下载闭环（本系统承担唯一证据库时为 P0） | SI、VGM、报关放行、提单确认件、异常证据不能在系统内可靠存取 | 手工从受控文件系统取得并填写 object key；当前未找到已验收流程 | 上传准备/完成确认、存储对象校验、受权下载/预览、审计；能关联节点/异常/真实单证 |
...
| 并发拆票失败请求返回错误语义 | 完整 PG 集成套件中只有一票成功且无重复写入，但失败方返回 400“分单数量不足”，而不是 409 版本冲突；用户无法按“刷新后重试”恢复，质量门失败 | Execute 的权威版本检查必须优先于依赖已变化业务状态的 Preview/门禁，或统一把并发导致的后置状态变化映射为稳定版本冲突；完整父测试重复通过 |
...
| 多个存量订单子资源查询/事务前预检使用 `r.data.db` 而不是 `Data.client(ctx)` | `order_milestone.go`、`order_abnormal_case.go`、`order_personnel.go`、`order_release_pod.go` 及 `order_fee.go` 的部分路径偏离当前共享事务规范；写事务的核心锁门禁多数仍使用 `tx`，暂未证明已造成业务错误，但增加组合事务与检查后竞态风险 |
