...
- [x] 成功用例通过真实 PostgreSQL 完成创建、提交后重读，并精确验证主单、明细、
      CNY 快照及创建审计。
- [x] 汇率失败、保存失败、审计失败三条路径均返回非空错误，且数据库中不存在本次
      提成主单、明细和创建审计。
- [x] 审计失败确实发生在真实仓储完成主单和明细写入之后，不是装饰器提前返回。
- [x] 默认环境未设置 `RONCIN_INTEGRATION_DATABASE_SOURCE` 时只有新集成测试明确
      跳过；显式注入隔离临时库后全部子用例通过且清理无残留。
- [x] `go -C server test ./internal/data -count=1`、`go -C server test ./...`、
      `go -C server vet ./...` 和 `git diff --check` 全部通过。
- [x] 不新增生产兜底、测试开关、API、迁移或生成物；若测试证明生产事务契约违约，
      停止并回到规划，不在测试任务中静默扩展生产修复。
- [x] 阶段 1、阶段 2、阶段 3和静态路由归档 PRD 的已完成验收框与现有证据一致。
