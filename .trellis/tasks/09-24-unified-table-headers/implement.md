...
- [ ] 确认 PRD 中无设置入口的表格是否新增能力，更新最终范围与验收。
- [ ] 盘点所有 Table、ProTable、EditableProTable 与封装模板，生成 research/entry-inventory.md，记录页面／入口、能力、存储、迁移结果。
...
5. 清理无调用的旧配置 UI；更新 component-guidelines.md 与 capability-navigation.md，逐项关闭迁移清单。
...
- 浏览器验证普通主数据、订单列表、订单费用应收／应付、财务费用增强设置和抽屉入口，覆盖空结果、长标题、必显／无权限列、固定列、保存失败、取消重开及两档屏幕尺寸；记录截图与检查结果。
