...
/**
 * 待审批注册「一键转派」模态：将误选本组织的申请转派至正确的目标公司。
 * 转派后原审批人不再收到待办，新公司审批人将收到通知。
 */
export default function RegistrationTransferModal({
  ...
  const candidateOrganizations = organizations.filter(
    (org) =>
      org.kind === AdminOrganizationKind.ORGANIZATION_KIND_COMPANY &&
      org.enabled !== false &&
      org.id &&
      org.id !== registration.requestedOrganizationId,
  );
  ...
  return (
    ...
    >
      <Alert
        ...
        title={
          <Space orientation="vertical" size={2}>
            <span>
              转派后将把该人员的申请组织更新为目标公司，并向目标公司的审批管理员发送钉钉通知。
            </span>
            <span style={{ fontSize: 12, color: '#8c8c8c' }}>
              当前自选组织：
              {registration.requestedOrganizationName || '系统管理收口'}
            </span>
          </Space>
        }
      ...
      <Space size={12} align="center" style={{ marginBottom: 20 }}>
        ...
        <div>
          ...
          <Text type="secondary" style={{ fontSize: 12 }}>
            当前归属：
            {registration.requestedOrganizationName || '未指定（系统管理收口）'}
          </Text>
        </div>
      </Space>
      ...
      <ProFormTextArea
        name="reason"
        label="转派原因"
        placeholder="请输入转派原因（例如：该员工属于天津港现场调度，由天津公司审核入职）"
