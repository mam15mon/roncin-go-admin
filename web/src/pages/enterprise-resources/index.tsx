...
const importConflictFieldLabels: Record<string, string> = {
  business_code: '企业代码',
  company_name: '企业名称',
};
...
const EnterpriseResourcesPage: React.FC = () => {
  ...
  useEffect(() => {
    if (editorOpen && active.type === 1 && countryCode === 'CN')
  ...
  return (
    <PageContainer title="配置管理" subTitle="组织级企业资源备忘录">
      ...
      <Modal
        title={associationMode === 'link' ? '批量关联企业' : '批量解除企业关联'}
      ...
      >
        <Select
          ...
          options={partnerOptions}
          value={associationPartners}
          onChange={setAssociationPartners}
          placeholder="搜索并选择企业"
        />
      </Modal>
      <Modal
        ...
        onCancel={() => setAddressTypeOpen(false)}
      >
        <Space orientation="vertical" style={{ width: '100%' }}>
          <Select
            value={addressTypeMode}
            onChange={setAddressTypeMode}
            options={[
      ...
      <Modal
        ...
        okText={
          !importPreview
            ? '校验数据'
            : (importPreview.conflictCount ?? 0) > 0
              ? `确认覆盖 ${importPreview.conflictCount} 条`
              : '确认导入'
        }
        ...
        okButtonProps={{
          disabled:
            ...
            ((importPreview?.conflictCount ?? 0) > 0 &&
              importPreview?.overwriteAllowed !== true),
        }}
        onOk={() =>
          importPreview ? void commitImport() : void previewImport()
        }
      ...
      >
        <Space orientation="vertical" size={12} style={{ width: '100%' }}>
          ...
          {!!importRows.length && !importPreview && (
            <Alert
              type="info"
              showIcon
              title={`已读取 ${importRows.length} 条数据，请先校验`}
            />
          )}
          {importPreview && (
            <Alert
              ...
              showIcon
              title={`有效 ${importPreview.validCount ?? 0} 条，无效 ${importPreview.invalidCount ?? 0} 条，冲突 ${importPreview.conflictCount ?? 0} 条`}
              description={
                    ...
                    .map((row) => (
                      <span key={`conflict-${row.rowNumber}`}>
                        ...
                        {row.conflicts
                          ...
                          .map(
                            (field) =>
                              importConflictFieldLabels[field] ?? field,
                          )
                          .join('、')}
                        ）
                        {(importPreview.conflictCount ?? 0) > 0 &&
