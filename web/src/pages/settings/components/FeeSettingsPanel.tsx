...
function TaxableServicesSubPanel() {
  const access = useAccess();
  const { message } = App.useApp();
  const actionRef = useRef<ActionType | undefined>(undefined);
  const [modalOpen, setModalOpen] = useState(false);
...
function AbnormalCasesSubPanel() {
  const access = useAccess();
  const { message } = App.useApp();
  const actionRef = useRef<ActionType | undefined>(undefined);
  const [modalOpen, setModalOpen] = useState(false);
...
export function FeeSettingsPanel() {
  ...
  const subItems = [
    {
      key: 'fee-items',
      label: '费用代码设置',
      children: <FeeItemsSubPanel />,
    },
    {
      key: 'billing-units',
      label: '计费单位',
      children: <BillingUnitsSubPanel />,
    },
    {
      key: 'taxable-services',
      label: '货物或应税劳务',
      children: <TaxableServicesSubPanel />,
    },
    access.canReadMasterDataItems
      ? {
          key: 'abnormal-cases',
          label: '异常情况',
          children: <AbnormalCasesSubPanel />,
        }
      : null,
  ...
  return (
    ...
    >
      <Tabs defaultActiveKey="fee-items" type="line" items={subItems} />
    </Card>
  );
}
