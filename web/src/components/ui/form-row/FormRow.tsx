 ...
 *
 * 基于 CSS Grid 的多列模板，跨行共享列边界（row-2 中线 = row-4 第二列边界 = row-6 第三列边界）：
 * - row-2 对半：一行两等分（对应旧 24 栅格 12/12）；
 * - row-3 三分：一行三等分（对应旧 8/8/8）；
 * - row-4 四分：一行四等分（对应旧 6/6/6/6）；
 * - row-5 五分：一行五等分；
 * - row-6 六分：一行六等分（对应 24 栅格 4/4/4/4/4/4，支持 1格/2格/3格 弹性跨列）。
...
export interface FormRowProps {
  /** 列数：2 对半 / 3 三分 / 4 四分 / 5 五分 / 6 六分，默认 4 */
  cols?: 2 | 3 | 4 | 5 | 6;
  className?: string;
...
export function FormRow({
  ...
  return (
    <GridContext.Provider value={NON_GRID_CONTEXT}>
      <div
        className={cx(styles.grid, styles[COLS_CLASS_MAP[cols]], className)}
        style={style}
      >
        {children}
      </div>
